...
from plex_pipe.config.config_schema import (
    DEFAULT_intensity_properties,
    DEFAULT_morphological_properties,
)
from plex_pipe.stages.quantification.metrics import METRIC_REGISTRY
...
class QuantificationController:
    ...
    def _build_signal_df(
        ...
        for channel_key in self.channels:
            img = self._get_channel(sdata, channel_key)
            for mask_suffix, mask in masks.items():
                logger.info(
    ...
    def _build_adata(self, sdata: sd.SpatialData) -> ad.AnnData:
        ...
        masks = self._get_masks_dictionary(sdata=sdata)
        ...
        quant_df = self._build_signal_df(sdata, masks)
    ...
    def run(
        ...
        # Handle overwiting
        self._prepare_to_overwrite(sdata=sdata)
        logger.info("Spatial Data object is valid and ready for quantification.")

        ########################################################################
        # Compute adata
        ########################################################################
        adata = self._build_adata(sdata=sdata)
        logger.info(
        ...
        ########################################################################
        sdata = self._connect_adata_to_mask(sdata=sdata, adata=adata)
        logger.info(
